...
def calculate_pct_change(orig, inc, dict):

    # boundary condition
    if ((orig >= 1960 or orig <= 2019) and (inc >= 1960 or inc <= 2019)):
        _original = dict[str(orig)]
        increase = dict[str(inc)] - _original
...
def main():
    ...
    while (original != -999):
        ...
        total_dollar = (original_amount*decimal_change) + original_amount
        ...
        if (change != -999):
            ...
            #print an error
            print("Make sure new_number enter a year between 1960 and 2019")
